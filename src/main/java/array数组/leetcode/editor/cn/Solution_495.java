 ...
 *
 * @author wangcy
 * @since  2022-06-06 15:10:17
 **/

  public class Solution_495{
      public static void main(String[] args) {

          Solution solution = new Solution_495().new Solution();
          int[] arr = {1,2};
          int poisonedDuration = solution.findPoisonedDuration(arr,2);
...
class Solution {
    public int findPoisonedDuration(int[] timeSeries, int duration) {
        ...
        for (int i = 0; i < timeSeries.length; i++) {
            int e = timeSeries[i]+duration-1;
            //上次攻击的时间是否和这次的
            ans += last < timeSeries[i] ? duration : e-last;
            last = e;
        }
        return ans;
    }
