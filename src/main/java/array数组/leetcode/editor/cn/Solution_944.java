 ...
 *
 * @author wangcy
 * @since  2022-07-06 14:36:32
 **/

  public class Solution_944{
      public static void main(String[] args) {

          Solution solution = new Solution_944().new Solution();
          String[] se = {"zyx","wvu","tsr"};
          int i = solution.minDeletionSize(se);
...
class Solution {
    public int minDeletionSize(String[] strs) {
        ...
        int index = 0;
        while (strList.size() < strs[0].length()){
            StringBuffer buffer = new StringBuffer();
            for (int i = 0; i < strs.length; i++) {
                buffer.append(strs[i].toCharArray()[index]);
            }
            index++;
