...
// 
// Related Topics 数组 哈希表 二分查找 排序 👍 199 👎 0
 ...
 *
 * @author wangcy
 * @since  2022-06-10 15:09:58
 **/

  public class Solution_888{
      public static void main(String[] args) {

          Solution solution = new Solution_888().new Solution();
          int[] aliceSizes = {1,2,5};
          int[] bobSizes = {2,4};
          int[] ints = solution.fairCandySwap(aliceSizes, bobSizes);
          for (int anInt : ints) {
...
class Solution {
    public int[] fairCandySwap(int[] aliceSizes, int[] bobSizes) {
        ...
        int avg = (aSum+bSum)/2;
        //a和b差的数值
        int aAndbdiff = avg - aSum;
        for (int aliceSize : aliceSizes) {
            if(funBool(bobSizes,aliceSize+aAndbdiff)){
                result[0] = aliceSize;
                result[1] = aliceSize+aAndbdiff;
            }
        }
        return result;
    ...
    public boolean  funBool(int[] b,int n){
        boolean flag = false;
        for (int i : b) {
            if(i == n){
                flag=true;
